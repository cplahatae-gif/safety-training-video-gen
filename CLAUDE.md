...
## 절대 원칙: 단계별 컨펌 없이 다음 단계 자동 진행 금지

**비용이 발생하는 단계(시나리오 / 캐릭터 시트 / 이미지 / 영상 / push)는 매 단계마다 사용자 명시 컨펌을 받고 진행한다.**

- "이전 단계가 잘 되었으니 다음도 자동으로" 같은 추론은 금지.
- 한 번에 여러 단계 묶어서 실행 금지 (예: 이미지 + 영상 동시 진행 금지).
- 사용자가 다음 단계를 명시할 때까지 멈춘 상태 유지.
- `FORCE_RUN=1`은 사용자가 명시적으로 지시할 때만 사용 (CI/자동화 스크립트 외).
- 컨펌 형식: 다음 액션이 무엇인지 1줄로 알려주고 "진행할까요?"로 묻는다.

**왜 이런 원칙이 있는가:** 과거에 자동 진행으로 토큰/$ 낭비가 발생했다. 시나리오나 이미지가 나쁜 채로 Kling(씬당 $0.50)을 돌리면 그대로 돈 날린다.

---

## 부속 원칙: Stage 5 진입 전 사람 리뷰 필수
